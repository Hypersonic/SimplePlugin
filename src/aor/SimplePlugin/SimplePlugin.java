	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args){
		...
		if(cmd.getName().equalsIgnoreCase("spellinfo")){ // If the sender typed /basic then do the following...
			if (sender instanceof Player) // If they're a player.
			{
				...
				{
					sender.sendMessage("Current spell (" + SimplePlugin.playerBooks.get(player.getName()).getCurrentSpell().getName() + "): " + SimplePlugin.playerBooks.get(player.getName()).getCurrentSpell().getDescription()); // Give them the current spell description.
				}
				else if (this.playerBooks.get(player.getName()).getSpell(args[0]) != null) // They gave an arg that matches a spel.
				{
					sender.sendMessage(this.playerBooks.get(player.getName()).getSpell(args[0]).getName() + ": " + this.playerBooks.get(player.getName()).getSpell(args[0]).getDescription()); // Give them the spell description.
				}
				else
				{
