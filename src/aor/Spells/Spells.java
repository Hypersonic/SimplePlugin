...
public class Spells extends JavaPlugin {
	...
	public void onEnable() {
		...
		{
			Spells.playerBooks.put(onlinePlayers[i].getName(), new SpellBook(onlinePlayers[i], this)); // Add a new spellbook for the player to the hashmap.
		}
		log.info("SimplePlugin enabled!");
	}
	public HashMap<String, SpellBook> getPlayerBooks()
	{
		return Spells.playerBooks;
	}
	...
	public boolean onCommand(CommandSender sender, Command cmd, String commandLabel, String[] args){
		if(cmd.getName().equalsIgnoreCase("spellinfo")) // If the command was /spellinfo
		{
			if (sender instanceof Player) // If they're a player.
			{
				...
				{
					if ((spellList.get(Spells.playerBooks.get(player.getName()).getCurrentSpell())).checkInventoryRequirements(player.getInventory())) // If they have the materials.
					{
						sender.sendMessage("Current spell " + ChatColor.DARK_GREEN + "(" + spellList.get(Spells.playerBooks.get(player.getName()).getCurrentSpell()).getName() + ")" + ChatColor.WHITE + ": " + spellList.get(Spells.playerBooks.get(player.getName()).getCurrentSpell()).getDescription()); // Give them the current spell description in green.
					}
					else // If they don't...
					{
						sender.sendMessage("Current spell " + ChatColor.DARK_RED + "(" + spellList.get(Spells.playerBooks.get(player.getName()).getCurrentSpell()).getName() + ")" + ChatColor.WHITE + ": " + spellList.get(Spells.playerBooks.get(player.getName()).getCurrentSpell()).getDescription()); // Give them the current spell description in red.
					}
				}
		...
		else if (cmd.getName().equalsIgnoreCase("listspells")) // If the command was /listspells
		{
			if (sender instanceof Player) // If they're a player.
			{
				...
				SpellBook playerBook = Spells.playerBooks.get(player.getName());
